"""Does averaging the vision estimate over consecutive frames reduce its error?
...
The 3-vs-more-frames question has two separate mechanisms. Higher-order
extrapolation (jerk) was measured and HURTS: perfect v+a gives 0.2707 and
perfect v+a+jerk gives 0.3674, because a third-order term diverges over a 3s
horizon. Noise reduction is the other one, and a least-squares model says it
wins big -- a RMSE drops 0.41x at 4 frames and 0.22x at 5.

That model assumes the per-frame estimation errors are INDEPENDENT. Adjacent
BEVs look at the same scene through overlapping observations, so they may not
be, and if the errors are correlated then averaging buys nothing and the
whole case for more frames collapses.

This measures the assumption on the real model instead of assuming it: take
the slot at consecutive frames of a scene, decode speed with a linear probe,
and compare the error of a single-frame estimate against an estimate averaged
over K consecutive frames.

Usage:
    python tools/probe_temporal_smoothing.py <eval_config> <ckpt> [--n 40]
"""
import argparse
