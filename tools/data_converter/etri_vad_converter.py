...
# (raw command.parquet never contains it) from the GT future trajectory,
# so it can only be assigned at train-pkl build time, never at test time
# (no future GT then). See STOP_DISP_THRESH below.
COMMAND_VOCAB = (
...
def robust_motion(times, positions, vel_window=4, accel_window=5):
    ...
    time. Callers must also only pass samples spaced TRAJ_STEP apart (see
    local_motion) -- the competition's inference rules require every
    timestamp whose ego-motion is used to also have that timestamp's image
    fed to the model, which raw-10Hz sample spacing would violate since
    only every TRAJ_STEP-th frame has a fed image. Velocity comes from a
...
def local_motion(data, index, rotation, accel_hops=4):
    ...
    Builds a window of up to `accel_hops` + 1 samples spaced TRAJ_STEP (5
    raw frames = 0.5s) apart, ending at `index` (never using index+1 or
    later) -- exactly the cadence at which images are fed to the model, so
    no timestamp without a corresponding fed image ever contributes here.
    Delegates to `robust_motion` for velocity/acceleration. rotation_rate
    stays a backward difference, now over one TRAJ_STEP hop instead of one
    raw frame for the same reason. Mirrors etri_test_converter.py's
    `local_motion`.
    """
    offsets = TRAJ_STEP * np.arange(accel_hops, -1, -1)
...
def ego_annotations(data, frame_id, ego_length, ego_width):
    ...
    # available and the ego barely moves in it -- see STOP_DISP_THRESH's
    # comment above for calibration against real data.
    fut_total_disp = (
