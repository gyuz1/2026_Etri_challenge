...
- [사용자 2026-09-17] "STOP 을 0~6m 로 하는 건 어때?" — [측정] STOP 선택 기준 TP 전방 < t (val, 칸별 train 평균 궤적 / 상태 입력 ridge 대리):
  ...
  [Claude 의견, 객관 재검토] **1m 유지 + STOP 라벨·TP≥1m frame 은 학습 때 STOP head 에 포함** 을 권함 (앞서 6m 찬성 답변을 정정).
  근거: (1) 대리로는 구분 불가(≤0.5%)이고 그 대리는 GT 가속도를 입력으로 써서, 비전이 못 보는 "곧 출발" 의도를 과대평가 → 6m 쪽으로 편향.
  (2) TP 1~6m 는 train 에서 28% 가 3초 정지·72% 가 이동 — 이 구분은 TP 만 알려주는 정보인데 6m 면 버림.
  (3) 영향 범위는 test 39 clip(3.5%)뿐, 26% 인 TP<1m 정지 clip 은 두 기준 모두 같게 처리. (4) 660 frame 은 정의상 3초 이동 <0.5m 라 STOP head 목표와 일치 → 제외·재생성 불필요.
  우선순위는 기준값보다 (a) 대조군에 TP<1m→STOP 선택 적용·측정, (b) 새 planner 구현·학습, (c) val 정지 비중 차이를 감안한 판단.
