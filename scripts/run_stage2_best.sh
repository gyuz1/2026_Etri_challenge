#!/usr/bin/env bash
# stage2 최선 구성. 사용법: ./scripts/run_stage2_best.sh <student-clean|nodistill-clean|goalpred-clean>
#
#   student-clean   -> A5000. 증류 student (제출 모델 후보). teacher epoch_12 필요.
#   nodistill-clean -> 3090.  증류 없는 대조군 (제출 가능).
#   goalpred-clean  -> 3090.  예측 5초 목표로 planning (nodistill-clean 과 goal_pred 만 다름).
...
case "$ROLE" in
  ...
  nodistill-clean)
    ...
    ;;
  goalpred-clean)
    # nodistill-clean + 예측 5초 목표(goal_pred). TP 는 라벨로만. config docstring 참조.
    ...
    INIT=work_dirs/stage1_best_nolcf/stage2_init_merged_lcfemb8.pth
    ;;
  *) echo "사용법: $0 <student-clean|nodistill-clean|goalpred-clean>" >&2; exit 1 ;;
esac
...
if grep -q "goal_pred=True" "$CONFIG"; then
  ...
  in_container $MACHINE "cd /workspace/VAD && CUDA_VISIBLE_DEVICES=0 python tools/check_goal_pred_live.py $CONFIG --n 6 --device 0" \
    || { echo "goal_pred 실측 검사 실패 -- 학습을 시작하지 않는다" >&2; exit 1; }
fi
