...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    def get_data_info(self, index, with_ann=True):
        ...
        Args:
            ...
            with_ann (bool): Whether to construct the annotation payload.
                History-only image queues can disable it because their labels
                are discarded by ``union2one``.
        ...
        input_dict = dict(
            ...
            # ``union2one`` converts this absolute pose to queue-relative
            # deltas in place.  Never alias the annotation payload: workers
            # revisit frames within and across epochs (especially with
            # persistent_workers), which would otherwise permanently corrupt
            # ``self.data_infos[index]['can_bus']``.
            can_bus=np.array(info['can_bus'], copy=True),
