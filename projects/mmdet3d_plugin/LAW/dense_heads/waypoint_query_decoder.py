...
class WaypointHead(BaseModule):
    ...
    def forward(self, img_feat, img_metas, ego_info=None, is_test=False):
        # init
        losses = {}
        Bz, num_views, num_channels, height, width = img_feat.shape #torch.Size([1, 6, 256, 12, 20])
        init_view_query_feat = self.view_query_feat.clone().repeat(Bz, 1, 1, 1).permute(0, 1, 3, 2) #randn torch.Size([1, 6, 6, 256])
        init_waypoint_query_feat = self.waypoint_query_feat.clone().repeat(Bz, 1, 1) #randn  torch.Size([1, 6, 256])

        # img pos emb
        img_pos = self.img_position_embeding(img_feat, img_metas) #from streampetr ,  torch.Size([1, 1440, 256])
        img_pos = img_pos.reshape(Bz, num_views, height, width, num_channels)  #torch.Size([1, 6, 12, 20, 256])
        img_pos = img_pos.permute(0, 1, 4, 2, 3) #torch.Size([1, 6, 256, 12, 20])
        ...
        spatial_view_feat = spatial_view_feat.reshape(batch_size, -1, num_channel) # torch.Size([1, 36, 256])
        ...
        cur_waypoint = self.waypoint_head(updated_waypoint_query_feat) # torch.Size([1, 6, 6])

        if self.num_traj_modal > 1:
            assert self.num_traj_modal == 3
            bz, traj_len, _ = cur_waypoint.shape #1 traj_len = 0.5 sec * 6 = 3sec 
            cur_waypoint = cur_waypoint.reshape(bz, traj_len, self.num_traj_modal, 2)
            ego_cmd = img_metas[0]['ego_fut_cmd'].to(img_feat.device)[0, 0] # 3 
            cur_waypoint = cur_waypoint[: ,: ,ego_cmd == 1].squeeze(2)  # torch.Size([1, 6, 3, 2])

        # world model prediction
        wm_next_latent = self.wm_prediction(spatial_view_feat, cur_waypoint)  # torch.Size([1, 36, 256])
    ...
    def wm_prediction(self, view_query_feat, cur_waypoint):
        ...
        action_aware_latent = self.action_aware_encoder(cur_view_query_feat_with_ego) #MLP

        wm_next_latent = self._wm_decoder(action_aware_latent, action_aware_latent)#Transformer decdoer
