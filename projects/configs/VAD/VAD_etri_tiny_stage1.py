...
#
# Stage 1 of the ETRI-native two-stage recipe: VAD-tiny perception/map/motion
# only, planning loss off, starting from the nuScenes LAW checkpoint. This
# adapts the (nuScenes-domain) pretrained backbone/BEV encoder/heads to the
# ETRI camera/scene domain before Stage 2 turns planning + the LAW world
# model on. See VAD_etri_tiny.py for the (previously single-stage) planning-
# enabled counterpart and VADLAW_etri_tiny.py for stage 2.
plugin = True
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        # Stage 1: perception/map/motion only -- planning loss off, matching
        # the original VAD stage1 recipe. The ego planning head still runs a
...
# nuScenes LAW checkpoint (stage2, 12ep, world model included). Object
...
# transfers. Plain VAD (this config) has no bev_world_model submodule, so
# those ~41 keys in the checkpoint are simply unused here -- they get
# reunited with this stage's output in the Stage-1->Stage-2 checkpoint merge.
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
             init_kwargs=dict(project='etri-2026-e2e-vad', name='stage1_split_301/75')),
    ])
checkpoint_config = dict(interval=1, max_keep_ckpts=total_epochs)

fp16 = dict(loss_scale=512.)

# Confirmed needed: without this, DDP crashes with "Expected to have
# finished reduction..." because some parameters (e.g. planning-loss-only
# branches, now permanently gated to 0) never receive gradient.
