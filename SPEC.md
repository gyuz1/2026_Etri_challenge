...
플래너가 먹는 슬롯은 **cosine 하나로만** 학습된다.

**무엇이 빠지는가는 측정으로 확정했다 (아래 probe 절).** 속도는 cosine 만으로도
들어간다(R² 0.985). **들어가지 않는 건 가속도다** — v1 슬롯의 `ax` R² 가 −0.023,
평균만도 못하다. 1 절 예산표에서 가속도를 전혀 모르는 상태(RMSE 0.46)는 L2 0.6866 이고,
완벽히 아는 상태는 0.2708 이다. 그 차이가 이 손실이 노리는 것 전부다.
...
### [측정 2026-09-13] v1 슬롯 probe — 근거가 속도가 아니라 **가속도** 였다

`tools/probe_status_slot.py`, v1 student(64차원 슬롯) 체크포인트, val 전역 80 샘플,
40/40 분할 선형 probe:

| | RMSE | 타깃 std | R² |
|---|---|---|---|
| speed | 0.6544 | 5.4133 | **0.985** |
| vx | 0.6541 | 5.4125 | 0.985 |
| vy | 0.0691 | 0.1507 | 0.790 |
| ay | 0.2073 | 0.2756 | 0.434 |
| yaw_rate | 0.0392 | 0.0434 | 0.186 |
| **ax** | 0.3599 | 0.3558 | **−0.023** |

**이전 주장 정정**: "cosine 은 스케일 불변이니 절대 속도가 슬롯에 안 들어갔을 것" 이라고
적었는데 **틀렸다.** 속도는 R² 0.985 로 잘 들어가 있다 — 채널 간 상대 관계로 스케일이
복원된 것으로 보인다.

**실제 공백은 가속도다.** `ax` 의 R² 가 **−0.023**, 즉 평균을 찍는 것보다 못하다.
슬롯에 가속도 정보가 없다. v1 은 2 프레임 descriptor 라 가속도가 **원리적으로 관측
불가능**했으므로 당연한 결과이고, v1 이 속도 오라클(0.5965)과 속도+가속도
오라클(0.2708) 사이인 0.4218 에 앉아 있는 것과 정확히 일치한다.

→ `ego_status_decode` 의 정당화는 속도가 아니라 **가속도** 다. 3 프레임이 가속도를
관측 가능하게 만들었고, decode 감독은 그게 **실제로 슬롯에 담기는지**를 보장한다.
관측 가능해진 것과 플래너가 먹는 슬롯에 들어가는 것은 별개다.

**한계**: n=80, 선형 probe 라 슬롯이 담은 정보의 **하한**이다(플래너는 비선형으로 더
뽑아낼 수 있다). v1 구조(64차원·2프레임)는 현재 학습 중인 것(8차원·3프레임)과 다르다.
