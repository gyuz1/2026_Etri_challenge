"""Stage-1 VAD, full-data variant: trains on all 376 scenes (no train/val
split) for the final submission run, where a held-out validation set is no
longer useful -- every scene we can train on should go into the submitted
model. Otherwise identical to VAD_etri_tiny_stage1_cached.py.
...
# Base config's WandbLoggerHook is named "stage1_split_301/75" -- override
...
log_config = dict(
    hooks=[
        ...
        dict(type='WandbLoggerHook',
             init_kwargs=dict(project='etri-2026-e2e-vad', name='stage1_fulldata')),
    ])
